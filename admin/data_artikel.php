...
ingin keluar?');" class="block text-red-600 hover:underline font-medium dark:text-red-300">Logout</a>
        ...
        <main class="w-3/4 bg-white rounded shadow p-6 ml-6 dark:bg-gray-700">
            <div class="flex justify-between items-center mb-4">
                ...
                <a href="add_artikel.php" class="bg-cyan-600 text-white px-4 py-2 rounded hover:bg-cyan-700
transition">+ Tambah Artikel</a>
            </div>
            <!-- Pencarian -->
            <div class="pt-8 px-4 pb-4">
                <div class="max-w-4xl mx-auto w-full">
                    <form action="data_artikel.php" method="GET" class="w-full flex gap-4">
            ...
            <table class="min-w-full table-auto border border-gray-300 text-sm">
                ...
                <tbody>
                    ...
                    if (isset($_GET['cari'])) {
                        $cari = $_GET['cari'];
                        $sql = "SELECT * FROM tbl_artikel WHERE nama_artikel LIKE '%$cari%'";
                    } else {
...
<a
href='edit_artikel.php?id_artikel={$data['id_artikel']}' class='text-cyan-600 hover:underline dark:text-cyan-300'>Edit</a>
<a
href='delete_artikel.php?id_artikel={$data['id_artikel']}'
onclick='return confirm(\"Yakin ingin menghapus?\")'
class='text-red-600 hover:underline dark:text-red-300'>Hapus</a>
</td>";
                        echo "</tr>";
                    }
                    ?>
