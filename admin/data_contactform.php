    ...
    <div class="flex max-w-7xl mx-auto mt-8 px-4">
        ...
        <aside class="w-1/4 bg-white rounded shadow p-4 dark:bg-gray-700">
            ...
            <ul class="space-y-2 text-gray-700 dark:text-white">
                ...
                <li><a href="data_contactform.php" class="block font-semibold text-cyan-700 dark:text-cyan-400">Contact
                        Form</a></li>
...
ingin keluar?');" class="block text-red-600 hover:underline font-medium dark:text-red-300">Logout</a>
        ...
        <main class="w-3/4 bg-white rounded shadow p-6 ml-6 dark:bg-gray-700">
            <div class="flex justify-between items-center mb-4">
                <h2 class="text-xl font-bold text-gray-800 dark:text-white">Daftar Pesan</h2>
            </div>
            <table class="min-w-full table-auto border border-gray-300 text-sm break-words">
                <thead class="bg-cyan-600 text-white">
                    <tr>
                        ...
                        <th class="p-3 border w-1/2">Isi Pesan</th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                    $sql = "SELECT * FROM tbl_contactform";
                    $query = mysqli_query($db, $sql);
                    $no = 1;
                    while ($data = mysqli_fetch_array($query)) {
                        echo "<tr class='even:bg-gray-50 dark:text-white dark:bg-gray-700'>";
                        echo "<td class='border p-2 text-center dark:text-white'>" . $no++ . "</td>";
                        echo "<td class='border p-2 dark:text-white'>" .
                            htmlspecialchars($data['nama_lengkap']) . "</td>";
                        echo "<td class='border p-2 dark:text-white'>" .
                            htmlspecialchars($data['email']) . "</td>";
                        echo "<td class='border p-2 dark:text-white'>" .
                            htmlspecialchars($data['no_hp']) . "</td>";
                        echo "<td class='border p-2 w-[300px] break-words dark:text-white'>" .
